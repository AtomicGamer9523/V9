...
impl ObjectTemplate {
  ...
  pub fn new<'s>(scope: &mut HandleScope<'s, ()>) -> Local<'s, ObjectTemplate> {
    unsafe {
      scope.cast_local(|sd| {
        v8__ObjectTemplate__New(sd.get_isolate_ptr(), core::ptr::null())
      })
    }
    .unwrap()
  }
  ...
  pub fn set_accessor_property(
    ...
    unsafe {
      let getter = getter.map_or_else(core::ptr::null, |v| &*v);
      let setter = setter.map_or_else(core::ptr::null, |v| &*v);
