...
impl<'a> core::ops::Deref for CachedData<'a> {
  type Target = [u8];
  fn deref(&self) -> &Self::Target {
    unsafe { core::slice::from_raw_parts(self.data, self.length as usize) }
  }
}
  ...
  pub fn new(
    ...
    unsafe {
      v8__ScriptCompiler__Source__CONSTRUCT(
        &mut buf,
        &*source_string,
        origin.map(|x| x as *const _).unwrap_or(core::ptr::null()),
        core::ptr::null_mut(),
      );
      buf.assume_init()
    }
