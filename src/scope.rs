...
pub(crate) mod data {
  ...
  impl ScopeData {
    ...
    /// all) in this case, which is why `core::alloc::alloc()` is used directly.
    #[cold]
    fn boxed(isolate: NonNull<Isolate>) -> Box<Self> {
      unsafe {
        #[allow(clippy::cast_ptr_alignment)]
        let self_ptr = malloc(Layout::new::<Self>()) as *mut Self;
        ptr::write(
          self_ptr,
          Self {
            isolate,
...
mod tests {
  ...
  fn initialize_v8() {
    ...
    INIT.call_once(|| {
      V9::initialize_platform(new_default_platform(0, false).make_shared());
      V9::initialize();
    });
  }
