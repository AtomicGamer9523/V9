    ...
    ) -> Local<'s, Value>,
{
  ...
  fn mapping() -> Self {
    let f = |ret_ptr, context, error, sites| {
      ...
      unsafe { core::ptr::write(ret_ptr, &*r as *const _) };
      ret_ptr
    };
    f.to_c_fn()
  }
...
/// A special hasher that is optimized for hashing `core::any::TypeId` values.
...
impl RawSlot {
  ...
  pub unsafe fn into_inner<T: 'static>(self) -> T {
    let value = if Self::needs_box::<T>() {
      *core::ptr::read(self.data.as_ptr() as *mut Box<T>)
    } else {
      core::ptr::read(self.data.as_ptr() as *mut T)
    };
    forget(self);
    value
  }
